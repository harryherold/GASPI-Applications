...
FILE * get_file_handle(const char * base_name);
void init_halos(uint64_t m, uint64_t n, size_t typesize);
void init_neightbours(uint64_t m, uint64_t n, size_t typesize);
gaspi_return_t read_halo(neighbour_t neighbour, halo_t halo_offset, seg_type_t seg_type, size_t typesize, gaspi_queue_id_t queue_id);

gaspi_return_t read_all_halos(seg_type_t seg_type, size_t type_size, gaspi_queue_id_t queue_other, gaspi_queue_id_t queue_south);
...
double compute_heat(double *u, double *v, size_t begin, size_t end, double residual);
